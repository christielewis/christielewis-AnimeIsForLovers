...
const getFormFields = (form) => {
  ...
  if (form.tagName !== 'FORM') {
    ...
    throw new Error(`Couldnt get form fields data. Expected a form, but instead got: ${submittedHtml} Please review '.md'`)
  }
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    if (isFormControl(e.tagName) && !e.hasAttribute('type')) {
      throw new Error(`Couldnt get form fields data. \n${formatHtml(e.outerHTML)}\n does not have a type. Please review '.md'`)
    }
